...
use Iwf2b\Field\Rule\RequiredUnlessRule;

class RequiredUnlessRuleTest extends \WP_UnitTestCase {
	...
	public function test_constructor_required_params() {
		...
		$this->expectExceptionMessage( 'The params "field" must be set for ' . RequiredUnlessRule::class );
	...
	public function test_constructor_fieldset_param_type() {
		...
		new RequiredUnlessRule( [ 'fieldset' => 'dummy_value' ] );
	}
	...
	public function test_constructor_field_param_type() {
		...
		new RequiredUnlessRule( [ 'field' => 10 ] );
	}
	...
	public function test_constructor_set_field_name_with_string() {
		...
		new RequiredUnlessRule( [ 'field' => 'field_name' ] );
	}
	...
	public function test_do_validation_with_field_object() {
		...
		$rule_1 = new RequiredUnlessRule( [ 'field' => $empty_field ] );
		...
		$rule_2 = new RequiredUnlessRule( [ 'field' => $non_empty_field ] );
	...
	public function test_do_validation_with_field_name_and_fieldset() {
		...
		$fieldset->shouldReceive( 'offsetExists' )->andReturnUsing( function ( $index ) {
			return ( $index === 'non_empty_field' || $index === 'empty_field' );
		} );
		...
		// Check with empty value and empty field
		$rule_1 = new RequiredUnlessRule( [ 'fieldset' => $fieldset, 'field' => 'empty_field' ] );
		$this->assertFalse( $rule_1->validate() );
		...
		// Check with empty value and non empty field
		$rule_2 = new RequiredUnlessRule( [ 'fieldset' => $fieldset, 'field' => 'non_empty_field' ] );
		$this->assertTrue( $rule_2->validate() );

		// Check with empty value when expected value does not match
		$rule_3 = new RequiredUnlessRule( [ 'fieldset' => $fieldset, 'field' => 'non_empty_field', 'expected' => 'invalid value' ] );
		$this->assertFalse( $rule_3->validate() );

		// Check with value when expected value does not match
		$rule_3->set_value( 'value' );
		$this->assertTrue( $rule_3->validate() );

		// Check with empty value when expected value matches
		$rule_4 = new RequiredUnlessRule( [ 'fieldset' => $fieldset, 'field' => 'non_empty_field', 'expected' => 'field value' ] );
		...
		$rule_5 = new RequiredUnlessRule( [ 'fieldset' => $fieldset, 'field' => 'unregistered_field' ] );
		$rule_5->validate();
	}
	...
	public function test_do_validation_strict() {
		...
		$rule_1 = new RequiredUnlessRule( [ 'fieldset' => $fieldset, 'field' => $field, 'expected' => '10', 'strict' => false ] );
		$this->assertTrue( $rule_1->validate() );

		// Check for strict mode with the expected value type does not match the field value type
		$rule_2 = new RequiredUnlessRule( [ 'fieldset' => $fieldset, 'field' => $field, 'expected' => '10', 'strict' => true ] );
		$this->assertFalse( $rule_2->validate() );

		// Check for not strict mode with value
		$rule_2->set_value( 'test' );
		$this->assertTrue( $rule_2->validate() );

		// Check for strict mode with the expected value type matches the field value type
		$rule_3 = new RequiredUnlessRule( [ 'fieldset' => $fieldset, 'field' => $field, 'expected' => 10, 'strict' => true ] );
		$this->assertTrue( $rule_3->validate() );
	}
}